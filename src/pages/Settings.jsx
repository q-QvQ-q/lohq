...
import ProfileAvatar, { getAvatarStoragePath } from '../components/ProfileAvatar.jsx'
...
export default function Settings() {
  ...
  async function handleAvatarChange(event) {
    ...
    try {
      ...
      if (oldPath && oldPath !== filePath && oldPath.startsWith(`${profile.id}/`)) {
      ...
      <div className="card">
        ...
        <div className="segmented-control" role="group" aria-label="外观模式">
          {Object.entries(THEMES).map(([key, data]) => (
            <button type="button" key={key} onClick={() => setTheme(key)} aria-pressed={theme === key}>
              <Icon name={key === 'light' ? 'sun' : 'moon'} size={17} />{data.name}
            </button>
          ))}
        </div>
      </div>
        ...
        {(profile?.partner_id || bindingStatus === 'bound') && (
          <div className="space-y-3">
            <div className="flex items-center gap-3 p-3 rounded-lg" style={{ backgroundColor: 'var(--color-primary-light)' }}>
              ...
              <span className="glass-pill text-xs font-semibold px-3 py-1">
                已绑定
              </span>
            </div>
            <button
          ...
          <div className="space-y-3">
            ...
            {bindingStatus === 'idle' && (
              <div className="text-center py-3">
                <Icon name="heart" size={23} className="mx-auto mb-1" />
                <p className="text-xs" style={{ color: 'var(--color-text-light)', opacity: 0.6 }}>
                  还没有绑定伴侣
                </p>
